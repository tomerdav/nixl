...
def worker(torch_rank: int, args: argparse.Namespace):
    ...
    # Initialize torch
    os.environ["CUDA_VISIBLE_DEVICES"] = str(local_rank % 2)
    ...
    os.environ["UCX_TLS"] = "rc,cuda_copy,gdr_copy"
    ...
    while True:
        ...
        # Keep failed ranks in the layout until they are disconnected.
        active_ranks_list = plan.get_active_ranks() + ranks_to_kill
...
def main():
    ...
    for i, p in enumerate(ctx.processes):
        ...
        if p.exitcode not in (0, -signal.SIGKILL):
            failed.append((i, p.exitcode))
    exits = [p.exitcode for p in ctx.processes]
    print(f"hard-fault exit codes: {exits}", flush=True)
    assert sorted(exits) == [-signal.SIGKILL, 0], exits
    if failed:
        raise RuntimeError(
