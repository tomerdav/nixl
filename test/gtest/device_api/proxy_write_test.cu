...
static nixl::proxyBackendOps
defaultOps() {
    nixl::proxyBackendOps ops;
    ops.init = [](const nixl::proxyConfig &) { return NIXL_SUCCESS; };
    ops.submit = [](const nixl::proxyBackendSubmission &, nixl::proxyBackendRequest &request) {
        request = {};
        return NIXL_SUCCESS;
    };
    ops.check_completion = [](const nixl::proxyBackendRequest &) { return NIXL_SUCCESS; };
...
TEST_F(ProxyDeviceApiTest, TokenLayoutAndGetPtr) {
    ...
    ASSERT_EQ(allocator->copy(device_view, &view, sizeof(view), nixl::deviceOps::copyDirection::HostToDevice), NIXL_SUCCESS);
    ...
    uint64_t result[3]{};
    ASSERT_EQ(allocator->copy(result,
                  output.get(),
                  sizeof(result),
                  nixl::deviceOps::copyDirection::DeviceToHost), NIXL_SUCCESS);
...
TEST_F(ProxyDeviceApiTest, ImmediatePutAndAtomicCompletionRoundTrip) {
    ...
    ASSERT_EQ(nixl::proxyRuntime::create(defaultOps(), makeProxyConfig(1, 1, 1), runtime),
              NIXL_SUCCESS);
...
TEST_F(ProxyDeviceApiTest, PutPutAtomicAddCompletionFrontier) {
    ...
    std::unique_ptr<nixl::proxyRuntime> runtime;
    ASSERT_EQ(nixl::proxyRuntime::create(backend.ops(), makeProxyConfig(1, 1, 1), runtime),
              NIXL_SUCCESS);
    ASSERT_EQ(runtime->startWorkers(), NIXL_SUCCESS);
    const DummyProxyMemViews views(*runtime);
...
TEST_F(ProxyDeviceApiTest, SubmitFailurePropagatesErrorStatus) {
    ...
    std::unique_ptr<nixl::proxyRuntime> runtime;
    ASSERT_EQ(nixl::proxyRuntime::create(ops, makeProxyConfig(1, 1, 1), runtime), NIXL_SUCCESS);
    ASSERT_EQ(runtime->startWorkers(), NIXL_SUCCESS);
    const DummyProxyMemViews views(*runtime);
    auto *result = deviceAlloc<DeviceResult>();
...
TEST_F(ProxyDeviceApiTest, PeerAndChannelRoutingKeepsCompletionsIndependent) {
    ControllableBackend backend;
    std::unique_ptr<nixl::proxyRuntime> runtime;
    ASSERT_EQ(nixl::proxyRuntime::create(backend.ops(), makeProxyConfig(2, 2, 2), runtime),
              NIXL_SUCCESS);
    ASSERT_EQ(runtime->startWorkers(), NIXL_SUCCESS);
