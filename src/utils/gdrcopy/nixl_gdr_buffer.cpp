...
nixlGdrBuffer::allocate(size_t count) {
    ...
    if (cudaMalloc(reinterpret_cast<void **>(&allocation_dev_), allocation_size_)
        != cudaSuccess) {
    ...
    if (slots_addr < info.va
        || slots_addr - info.va + data_size > info.mapped_size) {
...
nixlGdrBuffer::publish(size_t index, uint64_t value) noexcept {
    ...
    if (gdr_copy_to_mapping(
            mapping_handle_, slots_map_ + index, &value, sizeof(value))
        != 0) {
        return NIXL_ERR_BACKEND;
    }
#else
