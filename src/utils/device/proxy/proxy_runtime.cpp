...
nixl_status_t
nixlProxyChannelState::allocate(nixl::deviceOps &allocator,
                                uint32_t depth,
    ...
    if (allocator.copy(work_ring_mem_.get(),
                       &work_ring,
                       sizeof(work_ring),
                       nixl::deviceOps::copyDirection::HostToDevice) != NIXL_SUCCESS) {
        deallocate();
        return NIXL_ERR_BACKEND;
    }
    device_view = nixlProxyChannelView{static_cast<nixlProxyWorkRing *>(work_ring_mem_.get()),
